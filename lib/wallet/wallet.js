...
class Wallet extends EventEmitter {
  ...
  async createAuctionTxs(name, bid, lockup, options){
    const unlock = await this.fundLock.lock();
    try {
      return await this._createAuctionTxs(name, bid, lockup, options);
    } finally {
      unlock();
    }
  }
  ...
  async _createAuctionTxs(name, value, lockup, options){
    ...
    const reveal = await this.makeReveal(name,acct,force,bidBlind,bid.outputs[0].address);
    
    const coins = [];
    coins.push(Coin.fromTX(bid,0,this.wdb.height))
  ...
  async makeReveal(name, acct, force, blindBid,address) {
    assert(typeof name === 'string');
    ...
    if(blindBid){
      // const nameHashString = rules.hashName(name);
      // assert.equal(blindBid.ouputs[0].covenant.items[0],nameHashString);
  ...
  */
      ...
      // const blind = bidTx.ouputs[0].covenant.getHash(3);
  ...
  async _createReveal(name, options, bidBlind, address) {
    const acct = options ? options.account : null;
    const force = options ? options.force || false : false;
    const mtx = await this.makeReveal(name, acct, force, bidBlind, address);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }
  ...
  async createReveal(name, options, bidBlind) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._createReveal(name, options,bidBlind);
    } finally {
      unlock();
    }
  }
