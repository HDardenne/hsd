...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.post('/wallet/:id/bid-reveal', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const name = valid.str('name');
      const bid = valid.u64('bid');
      const lockup = valid.u64('lockup');
      ...
      const {
        bid: bidMTX,
        reveal: revealMTX
      } = await req.wallet.createAuctionTxs(name, bid, lockup, options);

      if (sign) {
        await req.wallet.sign(bidMTX, passphrase);
        await req.wallet.sign(revealMTX, passphrase);
      }

      return res.json(200, {
        bid: bidMTX.getJSON(this.network),
        reveal: revealMTX.getJSON(this.network)
      });
    });
