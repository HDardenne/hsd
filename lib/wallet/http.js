...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.post('/wallet/:id/bid-reveal', async (req, res) => {
      ...
      const options = TransactionOptions.fromValidator(valid);
      const {mtx, reveal} = await req.wallet.createAuctionTxs(name, bid, lockup, options);

      if (sign)
        await req.wallet.sign(mtx, passphrase);

      return res.json(200, {bid: mtx.getJSON(this.network), reveal:reveal.getJSON(this.network)});
    });
